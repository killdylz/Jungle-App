...
|---|---|---|
| **Dashboard** `:1821` | T (weak) | ✅ **DONE (session 9).** The KPI row is replaced pre-data by a three-step setup checklist (bring in classes → run first class → import attendance), gated on *sessions* rather than on checklist completion so a running gym never sees a setup card where its numbers belong. `src/lib/setupProgress.js` + `e2e/coldstart.spec.js`. AUTO-DJ card already sits behind `FLAGS.music`. "GOOD EVENING, COACH" kept. |
| **Class Builder** `:5215` | T | Left 60% is strong. Right column is music theatre (cut per AUDIT 2.1) — replace with the class summary + "Preview on TV". ~~ENERGY CURVE bar labels truncate~~ — **moot, verified 2026-07-25:** the ENERGY CURVE lives inside `subTab === "music"`, reachable only via the "Soundtrack" tab, which only renders when `FLAGS.music` is true (and `subTab` initialises to `"settings"` when it is false). The truncation is inside the music quarantine and unreachable in the shipping product. Note the curve is also a **hardcoded decorative SVG path**, identical for every class, under a label reading "peak intensity" — if music is ever un-quarantined, that is theatre and must be cut or made real before the labels matter. |
