...
3. ⭐ **CHUNK 2 — Extraction + generation Edge Functions** ← NEXT BUILD (I write code, user deploys + sets LLM key). `persona-extract`: deck text → `{blocks}` JSON → upsert `persona_plans` + recompute `persona_movements`. `persona-generate`: persona + class type + brief (focus, duration, +week X/N for Enduro) → LLM grounded on the CT profile + few-shot past plans + the movement-catalog vocabulary → `{blocks}` → Builder (coach edits + approves). Upgrades the current deterministic "Generate draft" to true in-style LLM generation. Model = latest Claude (see `claude-api` skill). Extend the existing `smart-build` Edge Function pattern.
4. **CHUNK 3 — Google Slides connector** (user's Google Cloud infra, I wire client). Enable Slides + Drive API; add `presentations.readonly` + `drive.readonly` to the OAuth consent screen — ⚠️ **sensitive scopes → likely re-triggers Google verification** on the already-published login OAuth app. Per-coach consent (personal accounts/folders). Client: the "Import from Google Slides" button on a persona → pick folder → `persona-extract`.
