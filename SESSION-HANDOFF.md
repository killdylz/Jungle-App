...
1. ⬜ **Apply `0005`** in Supabase (SQL Editor → paste → Run) — turns persona sync on. Now includes **`persona_movements`** (movement catalog) alongside `coach_personas` + `persona_plans`. Reviewed & sound; still NOT applied. Until then the UI runs on localStorage (hydrate degrades gracefully).
2. ✅ **CHUNK 1 DONE + verified (2026-07-14) — coach-first Personas UI.** `src/App.jsx`: rebuilt `PersonasScreen` (coach → **class-type tabs** → per-CT derived profile + editable movement catalog + plans), `PersonaProfilePanel`, `MovementCatalog` (rename folds a variant into `aliases`; equip/notes editable; counts+scheme derived), `PersonaPlanEditor` (full block/exercise editor — reorder, add/remove, schemes). `src/lib/personaAggregate.js` (NEW): `classTypesOf` / `aggregateClassType` / `aggregateMovements` — the derived-profile + catalog logic the Edge Function will mirror server-side. `src/lib/store.js`: `persona_movements` domain + `hydratePersonas` pulls all 3 tables. `src/data/personas.seed.js` reshaped to ONE sample **coach** with the S360 example plan. Verified live: seed → derived structure/schemes/defaults correct (rest medians, scheme counts) → 11-movement catalog auto-aggregated → movement rename folds alias (Assault Bike→Air Bike, count stays 1) → plan editor adds exercise → **save recomputes catalog 11→12** → Draft into Builder. Host build clean, zero console errors.
3. **CHUNK 2 — Extraction + generation Edge Function** (I write code, you deploy + set LLM key). `persona-extract`: deck text → `{blocks}` JSON → upsert `persona_plans` + recompute `persona_movements`. `persona-generate`: persona + class type + brief (focus, duration, +week X/N for Enduro) → LLM grounded on the CT profile + few-shot past plans + movement vocab → `{blocks}` → Builder. Upgrades "Generate draft" from deterministic-from-corpus to true LLM gen.
4. **CHUNK 3 — Google Slides connector** (your Google Cloud infra, I wire client). Enable Slides + Drive API; add `presentations.readonly` + `drive.readonly` to the OAuth consent screen — ⚠️ **sensitive scopes → likely re-triggers Google verification** on the already-published login OAuth app. Per-coach consent (personal accounts/folders). Client: "Import from Google Slides" → pick folder → `persona-extract`.

**Still TODO regardless: seed the 6 REAL Garage decks** — the seed is one illustrative coach; re-share the deck images to extract real `persona_plans` (deck data was NOT fabricated).
