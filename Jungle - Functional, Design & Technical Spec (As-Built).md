...
| **1 — Data foundation ★** | ✅ **Done** | Migrations `0001`–**`0008`** applied (**`0009` written, awaiting Dylan**); RLS on every table, self-tests for `0001`–`0006` **and** `0007` (11/11) written and run; Realtime room channels live; local-first sync across all 14 domains. F4 capture UI **shipped** (coach sweep + CSV backfill). **N4, the last Phase-1 gap, is now BUILT** (session 19) and waiting only on Dylan pasting two Edge Functions and one migration — `DYLAN-QUEUE.md` A12. |
...
| N4 | Member magic-link session summary | ✅ **Built (session 19)**, ⛔ deploy blocked on Dylan (`DYLAN-QUEUE.md` A12). The only member-facing surface |
...
| **N4 — member magic-link summary** | ✅ **BUILT (session 19); deploy blocked on Dylan** (`DYLAN-QUEUE.md` A12: one secret, two Edge Function pastes, migration 0009). **The only member-facing surface**, and the only place F6's white-label premium (A2) can be tested on a member. Built in the order the spec insisted on — token and Edge Functions first, page second. Verified by 58 unit tests and 17 e2e, including that the page renders with no app shell and no sign-in, that the token never appears in a URL or query string, and that a response carrying member-shaped fields renders none of them. |
...
| N4 | **Member magic-link summary** — ✅ **BUILT (session 19).** The only member-facing surface in the product, and the only place F6's white-label premium (A2) can be tested on a member. Both halves now exist: the share card (no backend) and the link. **Design:** `summary-token` (staff JWT, RLS *is* the authorization check) mints an HMAC-SHA256 token over `{class_instance_id, gym_id, exp}`; `summary-read` (no auth) validates it and reads with the service-role key. **RLS was not loosened to `anon` and 0007's policies were not touched** — the standing rule holds, and the Edge Function is the boundary LEGAL §4 said it should be. The token is **class-scoped, never member-scoped**: a leaked link exposes one class's programming — the same content the share card publishes to Instagram — and names nobody. Token core is one canonical module mirrored byte-for-byte into both functions, pinned by `classToken.mirror.test.js`. The page is routed **above `AuthGate`** in `main.jsx` (inside `App` it could never render, because AuthGate shows a login wall to anyone without a session), and lives on the URL **fragment**, not a query string, so the bearer token never reaches a server log or a `Referer` header. ⛔ **Blocked only on Dylan**: `DYLAN-QUEUE.md` **A12** (one secret, two function pastes, migration 0009) |
